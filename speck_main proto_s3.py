...
def new_window():
    w2= Toplevel()
    w2.geometry("400x400")
    myButton = Button(w2, text="Enter location:",padx=50, command= myclick,fg='blue',bg="light blue")
    myButton.pack()
    #if myButton.get()=="Enter Location":
        #myButton.delete(0,END)

    btn2=Button(w2, text="close window", command=w2.destroy).pack()
    '''bottom= Toplevel()
